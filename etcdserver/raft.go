...
const (
	...
	//假设RTT约为10ms，则1MB最大尺寸足够大。
	maxSizePerMsg = 1 * 1024 * 1024
	// Never overflow the rafthttp buffer, which is 4096.
	// 永远不要溢出rafthttp缓冲区，它是4096。
	// TODO: a better const?
	...
	go func() {
		...
		for {
			...
			case rd := <-r.Ready():
				...
				//领导者可以同时复制其磁盘，同时复制到跟随者，然后他们也可以写入磁盘。
				if islead {
...
func startNode(cfg ServerConfig, cl *membership.RaftCluster, ids []types.ID) (id types.ID, n raft.Node, s *raft.MemoryStorage, w *wal.WAL) {
	var err error
	//虽然没传本member的ID，但是cfg里面有name，能拿到。
	...
	c := &raft.Config{
		ID:              uint64(id),
		ElectionTick:    cfg.ElectionTicks, //选举时钟10s
		HeartbeatTick:   1,                 //心跳1s
		Storage:         s,
