...
func NewServer(cfg ServerConfig) (srv *EtcdServer, err error) {
	...
	//创建snap和wal储存文件目录，具体看函数注释，创建default.etcd目录
	if terr := fileutil.TouchDirAll(cfg.DataDir); terr != nil {
		return nil, fmt.Errorf("cannot access data directory: %v", terr)
	}

	haveWAL := wal.Exist(cfg.WALDir()) //cfg.WALDir() 返回的是 default.etcd/memeber/wal 目录。 wal.Exist 返回该目录下是否有.wal为后缀的文件

	if err = fileutil.TouchDirAll(cfg.SnapDir()); err != nil { //创建 default.etcd/memeber/snap 目录
		cfg.Logger.Fatal(
	...
	case !haveWAL && cfg.NewCluster: //正常第一次启动都是 没有WAL 并且 是一个新的cluster
		if err = cfg.VerifyBootstrap(); err != nil {
			return nil, err
		}
		// 初始化一个新的Cluster，里面记录了本集群的token和peer memeber 列表
		cl, err = membership.NewClusterFromURLsMap(cfg.Logger, cfg.InitialClusterToken, cfg.InitialPeerURLsMap)
		if err != nil {
			return nil, err
		}
		...
		// isMemberBootstrapped() 判断是否有同名的member已经启动过了。
		if isMemberBootstrapped(cfg.Logger, cl, cfg.Name, prt, cfg.bootstrapTimeout()) {
			return nil, fmt.Errorf("member %s has already been bootstrapped", m.ID)
		}
		//判断集群引导启动是否走服务发现。一般有3中集群启动方式：1. 配置引导 2. 服务发现引导 3. 还有个dns啥引导
		...
		cl.SetStore(st)
		cl.SetBackend(be)
		id, n, s, w = startNode(cfg, cl, cl.MemberIDs()) //把所有的memberID都传进去了,但是cfg中有本节点的name可以获取到本节点。
		cl.SetID(id, cl.ID())
...
func (s *EtcdServer) run() {
	...
	for {
		select {
		case ap := <-s.r.apply(): // 如果raftNode的applyc中有数据，就拿出来调用一下子。所以 applyc 是啥时候放入数据的呢？是在put操作send到其他节点之前放的。
...
func (s *EtcdServer) applyAll(ep *etcdProgress, apply *apply) {
	...
	proposalsApplied.Set(float64(ep.appliedi)) //监控
