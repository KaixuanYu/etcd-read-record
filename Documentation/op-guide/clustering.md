...
静态启动etcd集群要求每个成员都知道集群中的另一个成员。在许多情况下，群集成员的IP可能会提前未知。在这些情况下，可以在服务发现的帮助下引导启动etcd群集。
...
一旦etcd集群启动并运行，就可以通过运行时重新配置来添加或删除成员。为了更好地理解运行时重新配置背后的设计，我们建议阅读运行时配置设计文档。

This guide will cover the following mechanisms for bootstrapping an etcd cluster:

本指南将介绍用于引导etcd集群的以下机制：

* [Static](#static)  静态启动
* [etcd Discovery](#etcd-discovery) etcd 服务发现启动
* [DNS Discovery](#dns-discovery) dns 服务发现启动
...
|Name 名称|Address 地址|Hostname 域名|
|------|---------|------------------|
...
我们知道集群成员，它们的地址以及启动前集群的大小，因此我们可以通过设置 `initial-cluster` 标志来使用脱机引导程序配置。每台机器将获得以下环境变量或命令行：
...
请注意，`initial-cluster`中指定的URL是 _advertised peer URLs_ ，即它们应与相应节点上的 `initial-advertise-peer-urls` 值匹配。
...
如果出于测试目的使用相同的配置拆分多个群集（或创建和销毁单个群集），则强烈建议为每个群集分配唯一的 `initial-cluster-token` 。通过这样做，etcd可以为集群生成唯一的集群ID和成员ID，即使它们的配置完全相同。这样可以保护etcd免受跨集群交互的影响，因为跨集群交互可能破坏集群。
...
etcd侦听 `listen-client-urls` 接受客户端流量。 etcd成员将 `advertise-client-url` 中指定的URL通告给其他成员，代理，客户端。 请确保目标客户可以访问 `advertise-client-url`。 一个常见的错误是将advertise-client-urls设置为localhost，或者如果远程客户端应该访问etcd，则将其保留为默认值。
...
以 `--initial-cluster` 开头的命令行参数在etcd启动后的运行过程中会被忽略（就没用呗，就只在启动用到了）。在etcd启动之后，请随时删除环境变量或者命令行标识（意思就是没用了，删了无所谓）。如果需要更改配置（比如，增加或者删除节点），请参见 [runtime configuration][runtime-conf]
...
etcd支持通过TLS协议进行的加密通信。 TLS通道可用于对等体之间的加密内部群集通信以及加密的客户端流量。 本节提供了使用对等和客户端TLS设置群集的示例。 在[security guide][security-guide]中可以找到有关etcd的TLS支持的详细信息。
...
使用自签名证书的群集既可以加密流量，也可以验证其连接。 要使用自签名证书启动群集，每个群集成员应具有一个由共享群集CA证书（ca.crt）签名的唯一密钥对（member.crt，member.key），用于peer连接和client连接。 可以通过遵循 [TLS setup][tls-setup] 设置示例来生成证书
...
如果集群需要加密的通信但不需要经过身份验证的连接，则可以将etcd配置为自动生成其密钥。 初始化时，每个成员根据其通告的IP地址和主机创建自己的密钥集。
...
在以下示例中，我们没有将新主机包括在枚举节点列表中。 如果这是一个新集群，则必须将该节点添加到初始集群成员列表中。
...
$ etcd --name infra1 --initial-advertise-peer-urls http://10.0.1.11:2380 \
  ...
  --listen-client-urls http://10.0.1.11:2379,http://127.0.0.1:2379 \
  --advertise-client-urls http://10.0.1.11:2379 \
  --initial-cluster infra0=http://10.0.1.10:2380 \   // 说的是这里没有加上自己
  --initial-cluster-state new
...
在此示例中，我们尝试将节点（infra0）映射到与其在群集列表中枚举的地址（10.0.1.10:2380）不同的地址（127.0.0.1:2380）。 如果此节点要侦听多个地址，则所有地址都必须反映在“ initial-cluster”配置指令中。

```
$ etcd --name infra0 --initial-advertise-peer-urls http://127.0.0.1:2380 \   //这里出错，应该是10.0.1.10:2380
  --listen-peer-urls http://10.0.1.10:2380 \
  --listen-client-urls http://10.0.1.10:2379,http://127.0.0.1:2379 \
  --advertise-client-urls http://10.0.1.10:2379 \
  --initial-cluster infra0=http://10.0.1.10:2380,infra1=http://10.0.1.11:2380,infra2=http://10.0.1.12:2380 \
...
如果peer配置了一组不同的配置参数，并尝试加入此集群，则etcd将报告集群ID不匹配将退出。

意思是0和1成员的initial-cluster列表是0,1,2，但是3的initial-cluster列表为0,1,3导致的出错。
...
在许多情况下，群集对等体的IP可能不会提前知道。 使用云提供商或网络使用DHCP时，这很常见。 在这些情况下，请使用现有的etcd集群来引导新的集群，而不是指定静态配置。 此过程称为“发现”。
...
为了更好地了解发现服务协议的设计，我们建议您阅读发现服务协议 [文档][discovery-proto]。

#### Lifetime of a discovery URL [discovery URL 的生命周期]
...
一个 discovery URL 标识了唯一一个etcd 集群。每个etcd实例都都共享一个新的 discovery URL 来引导启动一个新集群，而不是复用一个已经存在的 discovery URL。
...
另外，discovery URL 应该只用于引导启动一个集群，集群启动后的集群变更请看 [runtime reconfiguration][runtime-conf]
...
discovery 使用现有群集来自我引导。 如果使用私有etcd集群，则创建如下URL：

```
//这里就是创建了一个key：value，就是告诉监听这个descovery的节点，该集群一共3个
$ curl -X PUT https://myetcd.local/v2/keys/discovery/6c007a14875d53d9bf0ef5a6fc0257c817f0fb83/_config/size -d value=3
```

By setting the size key to the URL, a discovery URL is created with an expected cluster size of 3.

通过给URL设置key的大小， discovery URL 被创建，并设置了期望集群成员的个数是3个。
...
```
// 只增加了--discovery的配置，该节点做了3件事：
// 1. 监听该discovery 
// 2. 往该discovery注册了自己 
// 3. 从该discovery知道了加入的集群中有3个节点，需要等这3个节点都全了之后才启动。
$ etcd --name infra0 --initial-advertise-peer-urls http://10.0.1.10:2380 \
  --listen-peer-urls http://10.0.1.10:2380 \
  --listen-client-urls http://10.0.1.10:2379,http://127.0.0.1:2379 \
  --advertise-client-urls http://10.0.1.10:2379 \
...
这将导致每个成员向自定义etcd发现服务进行注册，并在注册了所有计算机之后开始集群。
