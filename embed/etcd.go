...
const (
	...
	// 内部fd使用情况包括磁盘使用情况和传输使用情况。 fd应该是文件描述符的缩写
...
func StartEtcd(inCfg *Config) (e *Etcd, err error) {
	...
	//configurePeerListeners 获取了所有peer的listener
	if e.Peers, err = configurePeerListeners(cfg); err != nil { // e.Peers 就是一个监听器数组，监听器带有超时，读写都是5s
		return e, err
	}
	...
	//configureClientListeners 获取了所有的client的listener
	...
	defer func() {
		...
		for i := range peers {
			if peers[i] != nil && peers[i].close != nil {
				...
				ctx, cancel := context.WithTimeout(context.Background(), time.Second)
				peers[i].close(ctx) //如果有错误关闭listener
				cancel()
			}
		}
	}()

	for i, u := range cfg.LPUrls { //遍历listen peer urls  比如分别是:12380, :22380, :32380 初始化3个监听结构
		if u.Scheme == "http" {
			if !cfg.PeerTLSInfo.Empty() {
...
func configureClientListeners(cfg *Config) (sctxs map[string]*serveCtx, err error) {
	...
	for _, u := range cfg.LCUrls { //遍历listen client urls 像 :2379 , :22379, :32379
		...
		if fdLimit, fderr := runtimeutil.FDLimit(); fderr == nil {
			if fdLimit <= reservedInternalFDNum { //一般linux的fdLimit是1024，文件描述符限制。可以修改linux内核更改。
				cfg.logger.Fatal(
		...
		if network == "tcp" {
			//NewKeepAliveListener 实现了net.Listener，其中的Accept在net.Accept之后加了30s的keepalive
